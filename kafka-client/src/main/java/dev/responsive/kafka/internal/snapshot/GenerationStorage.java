...
/**
 * Interface for abstracting away how we lookup a task's current generation.
 * Ultimately we want to do one of the following so we can record a task's generation
 * metadata transactionally alongside the rows with new generation markers when we bump
 * generations:
