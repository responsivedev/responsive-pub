...
/**
 * Record offsets written to kafka and committed to consumer groups. When a commit happens, notify
 * listeners about the set of offsets.
 *
 * <p>The callbacks here need to be synchronized. Almost all the callbacks happen on the stream
...
class OffsetRecorder {
  ...
  private synchronized void onConsumedOffsets(
      final String consumerGroup, final TopicPartition partition, final long offset) {
  ...
  public static class RecordingKey {
    ...
    public RecordingKey(final TopicPartition partition, final String consumerGroup) {
