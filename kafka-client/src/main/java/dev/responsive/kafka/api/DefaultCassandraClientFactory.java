  ...
  public CqlSession createCqlSession(final ResponsiveConfig config) {
    final InetSocketAddress address =
        InetSocketAddress.createUnresolved(
            config.getString(STORAGE_HOSTNAME_CONFIG), config.getInt(STORAGE_PORT_CONFIG));
    ...
    return SessionUtil.connect(
        address, datacenter, tenant, clientId, clientSecret == null ? null : clientSecret.value());
  }

  @Override
  public CassandraClient createCassandraClient(
      final CqlSession session, final ResponsiveConfig responsiveConfigs) {
