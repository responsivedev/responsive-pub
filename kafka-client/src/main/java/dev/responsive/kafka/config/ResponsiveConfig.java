...
public class ResponsiveConfig extends AbstractConfig {
  ...
  private static final String CONNECTION_BUNDLE_DOC =
      "Path to the configuration bundle for "
          + "connecting to Responsive cloud. Either this or "
  ...
  private static final String TENANT_ID_DOC =
      "The tenant ID provided by Responsive for " + "resource isolation.";
  ...
  private static final String REQUEST_TIMEOUT_MS_DOC =
      "The timeout for making requests to the "
  ...
  public static final String REMOTE_TABLE_CHECK_INTERVAL_MS_CONFIG =
      "responsive.remote.table.check.interval.ms";
  private static final String REMOTE_TABLE_CHECK_INTERVAL_MS_DOC =
      "The frequency at which to poll for whether or not a remote table has been created. Mostly"
          + " used to speed up integration testing.";
  ...
  public static final String STORAGE_DESIRED_NUM_PARTITION_CONFIG =
      "responsive.storage.desired.num.partitions";
  private static final String STORAGE_DESIRED_NUM_PARTITIONS_DOC =
      "The desired number of partitions to create in the remote store. This is a best effort"
          + " target, as the actual number of partitions will be the next multiple of the Kafka"
          + " topic's number of partitions that is greater than or equal to this number. This"
          + " configuration does not apply to global stores. A value of -1 indicates to use the"
          + " number of Kafka Partitions as the remote partitions as well.";
  ...
  public static final String STORE_FLUSH_RECORDS_TRIGGER_CONFIG =
      "responsive.store.flush.trigger.local.records";
  public static final String STORE_FLUSH_RECORDS_TRIGGER_DOC =
      "The number of records to" + " accumulate in each store before flushing to remote";
  public static final int STORE_FLUSH_RECORDS_TRIGGER_DEFAULT = Integer.MAX_VALUE;

  public static final String STORE_FLUSH_BYTES_TRIGGER_CONFIG =
      "responsive.store.flush.trigger.local.bytes";
  public static final String STORE_FLUSH_BYTES_TRIGGER_DOC =
      "The size in bytes of buffered"
  ...
  public static final String STORE_FLUSH_INTERVAL_TRIGGER_MS_CONFIG =
      "responsive.store.flush.trigger.local.interval.ms";
  public static final String STORE_FLUSH_INTERVAL_TRIGGER_DOC =
  ...
  public static final String MAX_CONCURRENT_REMOTE_WRITES_CONFIG =
      "responsive.max.concurrent.writes";
  private static final String MAX_CONCURRENT_REMOTE_WRITES_DOC =
      "The maximum number of writes that will every be concurrently issued to the remote store."
          + " Increasing this value will reduce the time that it takes to flush data to the remote"
          + " store, but potentially increase the latency of each individual write and any other"
          + " concurrent reads from other threads. This configuration is 'local': in practice, the"
          + " number of concurrent writes from all applications to the remote store is this number"
          + " * the number of stream threads running across all nodes.";
  ...
  private static final ConfigDef CONFIG_DEF =
      new ConfigDef()
          ...
          .define(STORAGE_PORT_CONFIG, Type.INT, -1, Importance.HIGH, STORAGE_PORT_DOC)
          .define(
              STORAGE_DATACENTER_CONFIG,
              Type.STRING,
              null,
          ...
          .define(TENANT_ID_CONFIG, Type.STRING, Importance.HIGH, TENANT_ID_DOC)
          .define(
              CLIENT_ID_CONFIG,
              Type.STRING,
              null,
  ...
  public SubPartitioner getSubPartitioner(
      final Admin admin, final TableName name, final String changelogTopicName) {
    ...
    try {
      kafkaPartitions =
          admin
              .describeTopics(List.of(changelogTopicName))
              .allTopicNames()
              .get()
