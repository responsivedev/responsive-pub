...
public class DelayedAsyncStoreBuilder<T extends StateStore>
  ...
  private void maybeResolve() {
    if (innerResolved == null) {
      if (inner instanceof FactoryWrappingStoreBuilder) {
        innerResolved =
            (StoreBuilder<T>) ((FactoryWrappingStoreBuilder) inner).storeFactory().builder();
      } else {
        innerResolved = inner;
      }
    }
  ...
  private StateStore getKeyValueStore(final KeyValueStoreBuilder builder) {
    ...
    if (builder instanceof ResponsiveKeyValueStoreBuilder) {
  ...
  private StateStore getTimestampedKeyValueStore(
    ...
    final KeyValueBytesStoreSupplier storeSupplier;
    if (builder instanceof ResponsiveTimestampedKeyValueStoreBuilder) {
  ...
  private StateStore getSessionStore(final SessionStoreBuilder builder) {
    ...
    if (builder instanceof ResponsiveStoreBuilder.ResponsiveSessionStoreBuilder) {
