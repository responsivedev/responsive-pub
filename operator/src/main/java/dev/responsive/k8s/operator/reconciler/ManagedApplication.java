...
public abstract class ManagedApplication {
  ...
  public abstract void setReplicas(
      final Integer targetReplicas, final Context<ResponsivePolicy> context);

  public abstract String appType();

  public abstract String getResourceVersion();

  public abstract void maybeAddResponsiveLabel(
      Context<ResponsivePolicy> ctx, ResponsivePolicy policy);

  public static ManagedApplication build(Context<ResponsivePolicy> ctx, ResponsivePolicy policy) {
  ...
  private static void validateLabels(HasMetadata app, ResponsivePolicy policy) {
    ...
    assert labels.get(ResponsivePolicyReconciler.NAME_LABEL).equals(policy.getMetadata().getName());
    assert labels.containsKey(ResponsivePolicyReconciler.NAMESPACE_LABEL);
    assert labels
        .get(ResponsivePolicyReconciler.NAMESPACE_LABEL)
        .equals(policy.getMetadata().getNamespace());
  }

  private static ManagedApplication buildFromContext(
      Context<ResponsivePolicy> ctx, ResponsivePolicy policy) {
    ...
    if (!(isDeployment(appClient, namespace, appName)
      ...
      throw new RuntimeException(
          String.format("No deployment or StatefulSet %s/%s found", namespace, appName));
    }

    if (isDeployment(appClient, namespace, appName)) {
      final Deployment deployment =
          appClient.deployments().inNamespace(namespace).withName(appName).get();
      return new ManagedDeployment(deployment, appName, namespace);
    }

    final StatefulSet statefulSet =
        appClient.statefulSets().inNamespace(namespace).withName(appName).get();
  ...
  private static boolean isDeployment(AppsAPIGroupDSL appClient, String namespace, String appName) {
    final long matches =
        appClient.deployments().inNamespace(namespace).list().getItems().stream()
  ...
  private static boolean isStatefulSet(
      AppsAPIGroupDSL appClient, String namespace, String appName) {
    final long matches =
        appClient.statefulSets().inNamespace(namespace).list().getItems().stream()
