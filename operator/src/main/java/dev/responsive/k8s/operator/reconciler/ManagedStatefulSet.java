...
public class ManagedStatefulSet extends ManagedApplication {
  ...
  public ManagedStatefulSet(
      final StatefulSet statefulSet, final String appName, final String namespace) {
  ...
  public void setReplicas(final Integer targetReplicas, final Context<ResponsivePolicy> context) {
    context
        ...
        .edit(
            d -> {
              if (d.getMetadata().getResourceVersion().equals(getResourceVersion())) {
          ...
          .edit(
              d -> {
                if (d.getMetadata()
                  ...
                  newLabels.put(
                      ResponsivePolicyReconciler.NAME_LABEL, policy.getMetadata().getName());
                  d.getMetadata().setLabels(newLabels);
                }
                LOG.info(
                    "Added labels '{}:{}' and '{}:{}' to statefulset {}",
